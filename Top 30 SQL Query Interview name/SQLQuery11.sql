...
-- Q3. Write a query to get the current date.
 ...
 -- or 
 with cte as(select e1.emp_id,e1.f_name,e1.l_name ,e1.project ,e1.adress,e1.dob,e1.gender , row_number() over(partition by e1.emp_id order by e1.emp_id)as rn from empolyeeinfo_table e1
 join empolyee_position_table e2 on e1.emp_id=e2.emp_id ) 
select * from cte where rn <=1;

--or
with cte as(select e1.emp_id,e1.f_name,e1.l_name ,e1.project ,e1.adress,e1.dob,e1.gender from empolyeeinfo_table e1
 join empolyee_position_table e2 on e1.emp_id=e2.emp_id ),
 x as( select *, row_number() over(partition by emp_id order by emp_id)as rn  from cte)
select * from x where rn <=1;
 ...
 select department ,count(*)as co from empolyeeinfo_table
 group by department
 having count(*) >1;
...
select * from empolyeeinfo_table where emp_id>=(select count(*) from empolyeeinfo_table)-2;
 ...
 --or
 select * from (select *, row_number() over(order by emp_id)as rn from empolyeeinfo_table) as sp
 where rn=1 or rn=(select count(*) from empolyeeinfo_table);
